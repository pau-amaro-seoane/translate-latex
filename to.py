...
re_begin = re.compile(get_pattern("begin"))
re_end = re.compile(get_pattern("end"))
...
def repl_comment(obj):
    global ncomment
    ncomment += 1
    return f"___GTEXFIXCOMMENT{ncomment - 1}___"
...
recommand = re.compile(
    r"___GTEXFIXCOMMENT[0-9]*___|\\title|\\chapter\**|\\section\**|\\subsection\**|\\subsubsection\**|~*\\footnote[0-9]*|(\$+)(?:(?!\1)[\s\S])*\1|~*\\\w*\s*{[^}]*}\s*{[^}]*}|~*\\\w*\s*{[^}]*}|~*\\\w*"
)
commands = []
